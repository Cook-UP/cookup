...
class MenuUpload extends Component {
  ...
  // Handle when a file is selected to be uploated
  onFileChange = (e) => {
  ...
  uploadImage = async (e) => {
    ...
    await axios.get(API_ENDPOINT).then((res) => {
      ...
      console.log(
        "The url the picture can be found at is : ",
        response.uploadURL.split("?")[0]
      ); // this is the url
      ...
      console.log(
        "Hello the url of the image is saved in state as " + this.state.url
      );
  ...
  viewRestaurant = () => {
    this.props.history.push("/DisplayRestaurant");
  };
  render() {
          ...
          <Container>
            ...
            <StyledFormWrapper>
              <StyledForm onSubmit={this.mySubmitHandler}>
                {/* The form  */}
                <h1> Your Kitchen has been created now upload your photos</h1>
                <h3> Upload JPGs only </h3>
                {/* All of the input for the form below */}
                <label htmlFor="name">
                  {" "}
                  Upload the promotional photo for your Restaurant{" "}
                </label>
                <StyledInput
                  type="file"
                ...
                <label htmlFor="name">
                  Upload the Meunu of your Restaurant
                </label>
                <StyledInput
                  type="file"
                  name=" promotional photo"
                ...
                <br />
                <br />
                <Button onClick={this.viewRestaurant}>
                  View Your New Restaurant
                </Button>
              </StyledForm>
